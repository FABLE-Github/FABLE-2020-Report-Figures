...
#'}
PieDonut_FABLE=function(data,mapping,
  ...
  if(showRatioPie) {
    ...
    df$label=ifelse(df$ratio>=showRatioThreshold,
                    paste0(df$label,"\n",scales::percent(df$ratio, accuracy = 1),""),
  ...
  mainCol=c(wes_palette("Rushmore1", n = 5)[3:4], "#CDEAF5")
  ...
  if (unique(data$ALPHA3) %in% c("CHN", "IND")){
    lbl <- paste(round(sum(df$Freq)/1000), 'km$^{',3,'}/yr')
  }
  else{  lbl <- paste(round(sum(df$Freq)), 'Mm$^{',3,'}/yr')}
  
  if(showPieName) p1<-p1+annotate("text",x=0,y=0,label=TeX(lbl, output = "character") , parse = TRUE, size=titlesize,family=family)
