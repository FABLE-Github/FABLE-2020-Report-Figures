fct_Figure4 <- function(data, outpath, data_F4, country){
  ...
  #datalines dataframe only comprise data from the CT pathway, from 2010 to 2050, with pathway, emissions or removals from Crop, Livestock, Land use and biofuels
  datalines <- data %>% 
    select(Year, Pathway, CalcCropAllCO2e, CalcLiveAllCO2e, CalcAllLandCO2e, GHGbiofuels) %>% 
    slice(which(Pathway == "CT")) %>% 
    slice(which(Year %in% as.character(seq(2010, 2050, 5)))) %>% 
    pivot_longer(cols = c(CalcCropAllCO2e, CalcLiveAllCO2e, CalcAllLandCO2e, GHGbiofuels),
                 names_to = "type") %>% 
    data.frame()
  
  #duplicate values from CalcLandCO2e into CalcLandCO2eNeg to be able to plot emissions from land if it goes from positive to negative over the 2010 to 2050 period
  datalines <- rbind(datalines,
                     cbind.data.frame(
                       Year =  as.character(seq(2010, 2050, 5)),
                       Pathway = "CT",
